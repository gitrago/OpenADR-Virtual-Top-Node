  ...
  class Bot
    ...
    def chart_for(name)
      ...
      unless File.exist?(path)
        ...
        g.baseline_value = 0 if name == :voltage_change
        g.minimum_value = @sheet.result.send(name).delete(:minv)
        g.maximum_value = @sheet.result.send(name).delete(:maxv)
    ...
    def format_voltage_results
      ...
      text = []
      text << "\n---"
      text << "Maximum Voltage Change: #{format('**%0.2f%%**',
                                                (res[:vol_change_max][:val] * 100))} #{res[:vol_change_max][:label]}"
      text << "#{res[:vol_change_1][:label]}: #{format('**%0.2f%%**', (res[:vol_change_1][:val] * 100))} Min."
      text << "#{res[:vol_change_2][:label]}: #{format('**%0.2f%%**', (res[:vol_change_2][:val] * 100))} Max."
      text << "\n---"
      ...
      text << "Rapid Voltage Change (max. 3%): #{format('**%0.2f%%**',
                                                        (res[:rapid_vol_change][:val] * 100))} #{res[:rapid_vol_change][:label]}"
      text << "\n---"
    ...
    def coordinates_valid?
      ...
      if @coords[0].nil? || @coords[1].nil? then @error = :geocoding_error
      elsif !within_bound?(@coords[0], :latitude) then @error = :out_of_bounds
      elsif !within_bound?(@coords[1], :longitude) then @error = :out_of_bounds
      end
